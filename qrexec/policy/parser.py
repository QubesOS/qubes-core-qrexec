...
class VMToken(str, metaclass=VMTokenMeta):
    ...
    def __new__(cls, token: str, *, filepath: Optional[pathlib.Path]=None,
        ...
        if str_eq(token, "dom0") or str_eq(token, "uuid:00000000-0000-0000-0000-000000000000"):
            # TODO: log a warning in Qubes 4.1
            token = "@adminvm"

        # if user specified just qube name or UUID, use it directly
        if not (token.startswith("@") or str_eq("*", token)):
    ...
    def __eq__(self, other):
        """
        Trap if we ever try to compare a UUID with a non-UUID.
        This indicates a bug, so throw an exception.
        """
        if ((type(other) is not str) and
            (self.startswith("uuid:") != other.startswith("uuid:")) and
            not self.startswith("@") and
            not other.startswith("@")):
            raise RuntimeError("Cannot compare UUID and not-UUID")
