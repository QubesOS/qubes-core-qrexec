...
class TestAgentBase(unittest.TestCase):
    ...
    def assertExpectedStdoutStderr(self, target, expected_stdout: bytes,
                                   expected_stderr: bytes, *, exit_code: int=0) -> None:
        ...
        for msg_type, msg_body in messages[:-2]:
            match msg_type:
                case qrexec.MSG_DATA_STDOUT:
                    self.assertFalse(stdout_eof, "Got MSG_DATA_STDOUT after stdout EOF")
                    stdout_eof = not msg_body
                    stdout_entries.append(msg_body)
                case qrexec.MSG_DATA_STDERR:
                    self.assertTrue(msg_body, "Got empty MSG_DATA_STDERR before stderr EOF")
    ...
    def assertExpectedStdout(self, target, expected_stdout: bytes, *, exit_code=0):
        self.assertExpectedStdoutStderr(target, expected_stdout, b"", exit_code=exit_code)
