...
from qrexec.rpcconfirmation.admin_authz import (
    AdminAuthzRPCConfirmationWindow,
)
...
class MockAdminAuthzRPCConfirmationWindow(AdminAuthzRPCConfirmationWindow):
    def _new_focus_stealing_helper(self):
        return FocusStealingHelperMock(
            self._rpc_window, self._rpc_ok_button, self._focus_stealing_seconds
        )
...
@unittest.skipUnless(os.environ.get("DISPLAY"), "no DISPLAY variable")
class AdminAuthzRPCConfirmationWindowTestBase(GtkTestCase):
    def __init__(
        self,
        test_method,
        source_name="test-source",
        service="qubes.AuthorizeAdminAccess",
        argument="+",
        whitelist=["dom0"],
        target_name="dom0",
    ):
    ...
    def setUp(self):
        self.window = MockAdminAuthzRPCConfirmationWindow(
            self.test_source_name,
            self.test_service,
            self.test_argument,
            self.whitelist,
