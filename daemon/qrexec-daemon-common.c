...
static void wait_for_session_maybe(const char *cmdline)
{
...
static _Noreturn void handle_failed_exec(libvchan_t *data_vchan, bool is_service)
{
    int exit_code = 127;
    struct msg_header hdr = {
...
static _Noreturn void select_loop(struct handshake_params *params)
{
    ...
    _exit(params->exit_with_code ? exit_code : 0);
}
                        ...
                        bool exit_with_code)
{
    ...
    int prepare_ret = prepare_local_fds(remote_cmdline, &stdin_buffer);
...
_Noreturn void handshake_and_go(struct handshake_params *params)
{
    ...
    if (params->prepare_ret < 0)
        handle_failed_exec(params->data_vchan, params->remote_send_first);
